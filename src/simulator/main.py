...
from simulator.utils.create_components import (
    initialize_components,
    import_external_component,
)
from simulator.typehints.dict_types import (
    LogLevel,
    SystemArgs,
    Config,
    EntityDefinition,
...
class Simulator:
    ...
    def __init__(
        self,
        config: ConfigFormat = None,
        cleanup: CleanupFunction = lambda: print("Simulator Exited"),
    ):
        """
        ...
        if len(config_errors) > 0:
            ...
            raise SimulationParseError(
                f"Config from {self.CONFIG} could not be parsed."
            )
        else:
        ...
        simulation_components = (
            config.get("simulationComponents", None) if config is not None else None
        )
        ...
        if "loggerConfig" in self.simulator_extra_config:
            logger_config_file = (
                Path(context) / self.simulator_extra_config["loggerConfig"]
            )
            if logger_config_file.exists():
                logger.info(
                    f"Loading logger config file {logger_config_file.absolute()}"
                )
                ...
                logger.error(
                    f"Logger config file {logger_config_file.absolute()} not found"
                )
                self.build_report.append(
                    f"ERROR: Logger config file {logger_config_file.absolute()} not found"
                )
        ...
        if "map" in config:
            ...
            simulation = map_parser.build_simulation_from_map(
                file, simulation_components
            )
        else:
            self.build_report.append(
                "No map found in the configuration. Creating empty simulation."
            )
            simulation = map_parser.build_simulation_from_map(
                context, simulation_components, True
            )
        ...
        self.entities: List[Tuple[int, str]] = []
    ...
    def generate_simulation_build_report(self):
        ...
        self.build_report.append(
            f"{len(self.objects)} typed objects transformed into entities\n"
        )
        self.build_report.append(f"===> TYPED OBJECTS\n")
        for k, v in self.draw2ent.items():
            ent = v[0]
            ent_type = v[1].get("type", None)
            self.entities.append((ent, ent_type))

            if ent_type is None:
                continue
            self.build_report.append(
                f'• {k} --> esper entity {v[0]}. (type {v[1].get("type", "")})\n'
            )
    ...
    def add_system(self, system):
        ...
        if self.FPS == 0:
            logger.warning(
                f"Adding non-DES system {system}, but FPS was not given. System is useless."
            )
            self.build_report.append(
                f"WARNING: Useless non-DES system {system} because no FPS was provided."
            )
    ...
    def add_entity(self, entity_definition: EntityDefinition, ent_id: str):
        ...
        initialized_components = initialize_components(
            entity_definition.get("components", {})
        )
